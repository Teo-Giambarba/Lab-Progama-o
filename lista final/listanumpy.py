...
y = (x * 2) + (3 + np.random.random())
n = len(x)

# Formula dos minimos quadrados, ficou meio bagunçado mas é a representação literal da mesma
a = ((n * np.sum(x * y)) - (np.sum(x) * np.sum(y))) / ((n * np.sum(x ** 2)) - (np.sum(x) **2))
b = (np.sum(y) - a * (np.sum(x))) / n

print(a, b)
print(np.polyfit(x, y, 1))
# resultados são extremamente parecidos!!
